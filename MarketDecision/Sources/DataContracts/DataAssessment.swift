...
public struct CalculationInputReference: Sendable {
    ...
    public init(recordID: String, provenance: Provenance) { self.recordID = recordID; self.provenance = provenance }
}
/// Binds a computed number to exact input versions and the formula/model/parameter configuration.
/// This is reproducibility metadata, not evidence that the calculation or model passed validation.
public struct CalculationContext: Sendable {
    public let calculatedAt: Date
    public let formulaVersion: String, modelVersion: String, parameterVersion: String
    public let inputs: [CalculationInputReference]
    public init(calculatedAt: Date, formulaVersion: String, modelVersion: String, parameterVersion: String,
                inputs: [CalculationInputReference]) throws {
        guard finite(calculatedAt), nonblank(formulaVersion), nonblank(modelVersion), nonblank(parameterVersion),
              !inputs.isEmpty else { throw ContractError.invalidIdentity }
        for input in inputs {
        ...
        self.calculatedAt = calculatedAt; self.formulaVersion = formulaVersion; self.modelVersion = modelVersion
...
public struct NumericObservation: ProviderRecord, Sendable {
    ...
    public init(recordID: String, provenance: Provenance, rawValue: String?, value: Money?, state: ValueState,
                ...
                calculation: CalculationContext? = nil) throws {

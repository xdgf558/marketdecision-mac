...
- Exact Money arithmetic is retained through calculations; no automatic posting or binary-price conversion. Output pins the resolved model/parameter references, dictionary, source versions, normalized inputs, class evidence and execution/as-of times.
...
- Every expected class requires its own actual-share and price evidence on a consistent share-count date. Missing classes withhold capitalization; consolidated EPS is not applied across classes. Multiclass price inversion is unavailable without a separately implemented ratio-evidence contract.
...
- Clean public-copy local verification passed: 214 declarations, 213 executed tests and one real Keychain test NOT EXECUTED; 24 new tests in four suites. Debug/Release builds, effective permissions, Release isolation, 12 traceability and four UTC checks passed on macOS 27 / Xcode 27 beta 6 / arm64. Hosted CI has not run for this unpublished candidate.
